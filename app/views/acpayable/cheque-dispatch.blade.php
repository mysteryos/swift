...
<!-- MAIN CONTENT -->
<div id="content" data-js="acp_cheque_dispatch" data-urljs="{{Bust::url('/js/swift/swift.acp_cheque_dispatch.js')}}">
    <div class="row">
        <div class="col-md-4 col-lg-2 col-xs-12">
            <h1 class="page-title txt-color-blueDark hidden-tablet"><i class="fa fa-fw fa-envelope"></i> Cheque Dispatch &nbsp;</h1>
        </div>
            ...
            <div class="ribbon-button-alignment page-title">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li>
                            <a href="javascript:void(0);" class="btn-tick-no-dispatch">No Dispatch</a>
                        </li>
                        <li class="divider"></li>
                        <li>
                ...
                <div class="btn-group toggle-oncheck"style="display:none;">
                    <button class="btn btn-default" id="btn-setbatchdispatch" data-original-title="Set Dispatch" data-placement="bottom" rel="tooltip">
                ...
                <div class="col-xs-12 inbox-side-bar">
                    ...
                    <ul class="inbox-menu-lg">
                            <li @if($type=="all"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/all" class="form-pjax-filter pjax"><i class="fa fa-file-text-o"></i>All @if($all_count > 0 ){{"(".$all_count.")"}} @endif</a>
                            </li>
                            <li @if($type=="overdue"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/overdue" class="form-pjax-filter pjax"><i class="fa fa-clock-o"></i>Overdue @if($overdue_count > 0 ){{"(".$overdue_count.")"}} @endif</a>
                            </li>
                            <li @if($type=="today"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/today" class="form-pjax-filter pjax"><i class="fa fa-check"></i>Today @if($today_count > 0){{"(".$today_count.")"}} @endif</a>
                            </li>
                            <li @if($type=="tomorrow"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/tomorrow" class="form-pjax-filter pjax"><i class="fa fa-reply"></i>Tomorrow @if($tomorrow_count > 0){{"(".$tomorrow_count.")"}} @endif</a>
                            </li>
                            <li @if($type=="future"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/future" class="form-pjax-filter pjax"><i class="fa fa-calendar"></i>Future @if($future_count){{"(".$future_count.")"}} @endif</a>
                            </li>
                            <li @if($type=="nodate"){{"class=\"active\""}}@endif >
                                    <a href="/{{ $rootURL }}/cheque-dispatch/nodate" class="form-pjax-filter pjax"><i class="fa fa-question"></i>No Due Date @if($nodate_count){{"(".$nodate_count.")"}} @endif</a>
                            </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="col-md-8 col-lg-10 col-xs-12">
                    ...
                    @if($filter_on)
                        <div class="hidden-tablet pull-left">
                            ...
                            @foreach($filter as $name => $f)
                                @if($f['enabled'])
                                    <a href="{{"/".$rootURL."/cheque-dispatch/".$type."/0".Helper::filterQueryParam(Url::full(),$name)}}" class="btn btn-sm btn-default pjax">{{$f['name'].": ".$f['value']}} <i class="fa fa-times"></i></a>
                                @endif
                            @endforeach
                            <a href="{{"/".$rootURL."/cheque-dispatch/".$type."/0"}}" class="btn btn-sm btn-default pjax">Clear All <i class="fa fa-times"></i></a>
                        </div>
                    @endif
                </div>
                <div class="col-xs-4">
                    ...
                    <div class="btn-group pull-right inbox-paging">
                            <a href="@if($page == 1){{"javascript:void(0);"}}@else{{"/".$rootURL."/cheque-dispatch/".$type."/".($page-1).$filter_string}}@endif" class="btn btn-default btn-sm @if($page == 1){{"disabled"}}@else{{"pjax"}}@endif" id="inbox-nav-previous"><strong><i class="fa fa-chevron-left"></i></strong></a>
                            <a href="@if($page == $total_pages){{"javascript:void(0);"}}@else{{"/".$rootURL."/cheque-dispatch/".$type."/".($page+1).$filter_string}}@endif" class="btn btn-default btn-sm @if($page == $total_pages){{"disabled"}}@else{{"pjax"}}@endif" id="inbox-nav-next"><strong><i class="fa fa-chevron-right"></i></strong></a>
                    </div>
                    @endif
            ...
            <div class="row">
                <div class="col-xs-12 table-wrap custom-scroll animated fast fadeInRight">
                    @include('acpayable.cheque-dispatch-list')
                    @include('acpayable.dispatch-select')
                </div>
            </div>
        </div>
    </div>
