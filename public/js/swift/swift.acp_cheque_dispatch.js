/* 
 * Name: ACP Cheque Dispatch
 * Description:
 */

(window.acp_cheque_dispatch = function(){
    var $content = $('#content');
    tableHeightSize();
    ...
    $content.on('click','.btn-tick-no-dispatch',function(){
        $content.find('tr.pvform').removeClass('highlight')
                .find('input[type="checkbox"]').prop("checked",false);
        var $pvlist = $content.find('.input-dispatch').filter(function(){
    ...
    $('#btn-setbatchdispatch').on('click',function(){
        $('#assignDispatchModal').modal({
            backdrop: false
        });
        return false;
    ...
    function setbatchdispatch()
    {
        $('tr.pvform.highlight').find('.input-dispatch').val($('#batchSelectDispatch').val());
        $('tr.pvform.highlight').find('.input-dispatch').each(function(){
            saveInput($(this));
        });
    }
    ...
    $('#inbox-table').on('change','.input-dispatch',function(e){
        saveInput($(this));
    });
    ...
    $('#assignDispatchModal').on('shown.bs.modal', function(){
        //Reset Inputs
        $('#batchSelectDispatch')[0].selectedIndex = 0;       
    });
    
    $('#btn-saveDispatch').on('click',function(){
       if($('#batchSelectDispatch')[0].selectedIndex === 0)
       {
       ...
       setbatchdispatch();
       $('#assignDispatchModal').modal('hide');
       return false;
    });
