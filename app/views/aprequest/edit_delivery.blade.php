<fieldset data-name="erporder" class="fieldset-erporder multi @if(isset($dummy) && $dummy == true) dummy hide @endif ">
    ...
    <div class="row">
        <div class="form-group col-lg-6 col-xs-12">
            ...
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_ref_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$isStore && !$isAdmin) editable-disabled @endif" data-type="text" data-context="delivery" data-name="invoice_number" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ Crypt::encrypt($form->id) }}" data-value="{{ $d->invoice_number or "" }}"></a>
            </div>                                                                                        
        </div>
        <div class="form-group col-lg-6 col-xs-12">
            <label class="col-md-2 control-label">Invoice Recipient</label>
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_ref_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$isStore && !$isAdmin) editable-disabled @endif" data-type="text" data-context="delivery" data-name="invoice_recipient" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ Crypt::encrypt($form->id) }}" data-value="{{ $d->invoice_recipient or "" }}"></a>
            </div>                                                                                        
        </div>        
    </div>    
    <div class="row">
        <div class="form-group col-lg-6 col-xs-12">
            ...
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_delivery_person_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$isStore && !$isAdmin) editable-disabled @endif" data-type="text" data-context="delivery" data-name="delivery_person" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ Crypt::encrypt($form->id) }}" data-value="{{ $d->delivery_person or "" }}"></a>
            </div>                                                                                        
        </div>
        <div class="form-group col-lg-6 col-xs-12">
