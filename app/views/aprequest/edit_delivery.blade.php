<fieldset data-name="delivery" class="fieldset-delivery multi @if(isset($dummy) && $dummy == true) dummy hide @endif ">
    ...
    <div class="row">
        <div class="form-group col-lg-6 col-xs-12">
            ...
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_ref_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$permission->isStore() && !$permission->isAdmin()) editable-disabled @endif" data-type="text" data-context="delivery" data-name="invoice_number" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ $form->encrypted_id }}" data-value="{{ $d->invoice_number or "" }}"></a>
            </div>                                                                                        
        </div>
        <div class="form-group col-lg-6 col-xs-12">
            <label class="col-md-2 control-label">Invoice Recipient</label>
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_ref_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$permission->isStore() && !$permission->isAdmin()) editable-disabled @endif" data-type="text" data-context="delivery" data-name="invoice_recipient" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ $form->encrypted_id }}" data-value="{{ $d->invoice_recipient or "" }}"></a>
            </div>                                                                                        
        </div>        
    </div>    
    <div class="row">
        <div class="form-group col-lg-6 col-xs-12">
            ...
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_delivery_person_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$permission->isStore() && !$permission->isAdmin()) editable-disabled @endif" data-type="text" data-context="delivery" data-name="delivery_person" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ $form->encrypted_id }}" data-value="{{ $d->delivery_person or "" }}"></a>
            </div>                                                                                        
        </div>
        <div class="form-group col-lg-6 col-xs-12">
            <label class="col-md-2 control-label">Delivered On</label>
            <div class="col-md-10">
                <a href="#" @if(isset($d->id)) {{ "id=\"delivery_delivery_date_".Crypt::decrypt($d->id)."\"" }} @endif class="editable delivery-editable @if(isset($dummy) && $dummy == true) dummy @endif @if(!$permission->isStore() && !$permission->isAdmin()) editable-disabled @endif" data-type="date" data-viewformat="dd/mm/yyyy" data-date="@if(isset($d->delivery_date)){{$d->delivery_date->format('d/m/Y')}}@endif" data-context="delivery" data-name="delivery_date" data-pk="{{ $d->id or 0 }}" data-url="/{{ $rootURL }}/delivery/{{ $form->encrypted_id }}">@if(isset($d->delivery_date)){{$d->delivery_date->format('d/m/Y')}}@endif</a>
            </div>                                                                                        
        </div>        
    </div>
    <legend class="top"></legend>
    @if($edit && ($permission->isStore() || $permission->isAdmin()))<a class="btn btn-default btn-xs top-right btn-delete" href="/{{ $rootURL }}/delivery" title="Delete Delivery"><i class="fa fa-trash-o"></i></a>@endif
